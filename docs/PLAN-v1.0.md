...
| **v0.6.0** (완료) | **Multi-Session Hardening (P7)** — BL-ID 네임스페이스 격리, session registry, atomic-write, seed/restore 가드, config 분리, per-session codex socket | Portal Hub BL-306 실측 대응 (HARNESS-MSC-001) ✅ 2026-04-20 |
| **v0.7.0** (완료) | **Starter templates (P8)** (`templates/overrides-starter/`) + README 커스터마이징 가이드 + FAQ | 신규 사용자 onboarding 5분 kit ✅ 2026-04-20 |
...
> 다음 액션: v0.8.0 — `/harness:resume` 본체 구현 + Evaluation criteria override 전용 파이프라인.
