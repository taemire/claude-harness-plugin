...
## 0단계: BL-ID 해결 (v0.6 신규 — HARNESS-MSC-001 P0-1)

하네스 산출물 충돌 방지를 위해 **모든 경로는 `<BL-ID>` 네임스페이스로 격리**됩니다. 파이프라인 진입 직후 아래 절차로 BL-ID 를 해결합니다.

```bash
# 1. 사용자 요청 원문에서 BL-NNN 추출 시도
BL_ID=$(bash ${CLAUDE_PLUGIN_ROOT}/.harness/common/extract_bl_id.sh "[사용자 요청 원문]" \
        || bash ${CLAUDE_PROJECT_DIR}/.harness/common/extract_bl_id.sh "[사용자 요청 원문]")

# 2. BL_ID 에 따라 파이프라인 내 모든 <BL-ID> 자리표시자를 실제 값으로 치환
#    예) .harness/feature/<BL-ID>/SPEC.md → .harness/feature/BL-306/SPEC.md
```

**해결 우선순위** (`extract_bl_id.sh` 가 자동 수행):
1. 요청 원문의 첫 번째 `BL-<digits>`
2. `git log --oneline -n 10` 첫 번째 `BL-<digits>`
3. `gh issue list` 첫 번째 `BL-<digits>` (GH_DISABLED=1 이면 skip)
4. fallback: `BL-UNKNOWN-YYYYMMDD-HHMM`

**Legacy flat layout fallback (v0.5 호환)**:
- 본 스킬이 진입 시 `.harness/<type>/SPEC.md` (BL-ID 없는 평면 경로) 이 존재하면 해당 **레거시 세션으로 간주** 하여 새 경로 생성 없이 기존 경로를 그대로 사용합니다. (v0.7 에서 deprecated 경고, v1.0 에서 제거 예정)

**활성 세션 등록 (P0-2)**:
BL-ID 해결 후, plugin 의 `hooks/session-start.sh` 가 이미 `.harness/active-sessions.json` 에 엔트리를 등록했을 것입니다. 추가로 phase 필드를 갱신하려면:

```bash
bash ${CLAUDE_PLUGIN_ROOT}/common/session-registry.sh register "$BL_ID" "planner"
```

**멀티세션 경고 확인 (HARNESS-MSC-001)**:
session-start.sh 의 L5 preflight 가 타세션 활성을 이미 감지하여 경고를 출력했을 것입니다. 경고가 있었다면 파이프라인 시작 전 **사용자에게 go/no-go 확인** 을 받아야 합니다:

```
⚠️ 타세션 감지 — 동일 프로젝트의 다른 Claude 세션이 .harness/<type>/ 를 점유 중일 수
   있습니다. 진행하시겠습니까? (BL-ID 네임스페이스가 다르면 안전합니다.)
```

무음 또는 애매한 답변은 no-go 로 간주합니다.
...
결과를 ${CLAUDE_PROJECT_DIR}/.harness/ui/<BL-ID>/SPEC.md 로 저장하라.
```
...
```
Read ${CLAUDE_PROJECT_DIR}/.harness/ui/<BL-ID>/SPEC.md
...
결과를 ${CLAUDE_PROJECT_DIR}/.harness/ui/<BL-ID>/SPEC.md 파일 끝에
## Codex Plan Review
...
- ${CLAUDE_PROJECT_DIR}/.harness/ui/agents/evaluation_criteria.md
- ${CLAUDE_PROJECT_DIR}/.harness/ui/<BL-ID>/SPEC.md

프로젝트 컨텍스트:
- ${CLAUDE_PROJECT_DIR}/web/src/components/ui/ 의 shadcn 컴포넌트 목록 확인
- ${CLAUDE_PROJECT_DIR}/web/src/store/ 스토어 구조 확인

전체 기능을 구현하고 결과를 ${CLAUDE_PROJECT_DIR}/.harness/ui/<BL-ID>/output/ 에 저장하라.
완료 후 ${CLAUDE_PROJECT_DIR}/.harness/ui/<BL-ID>/SELF_CHECK.md 를 작성하라.
```
...
- ${CLAUDE_PROJECT_DIR}/.harness/ui/agents/evaluation_criteria.md
- ${CLAUDE_PROJECT_DIR}/.harness/ui/<BL-ID>/SPEC.md
- ${CLAUDE_PROJECT_DIR}/.harness/ui/<BL-ID>/output/ 의 모든 파일

검수 후 결과를 ${CLAUDE_PROJECT_DIR}/.harness/ui/<BL-ID>/QA_REPORT.md 로 저장하라.
```
...
- ${CLAUDE_PROJECT_DIR}/.harness/ui/agents/evaluation_criteria.md
- ${CLAUDE_PROJECT_DIR}/.harness/ui/<BL-ID>/SPEC.md
- ${CLAUDE_PROJECT_DIR}/.harness/ui/<BL-ID>/output/ 의 현재 파일들 (현재 코드)
- ${CLAUDE_PROJECT_DIR}/.harness/ui/<BL-ID>/QA_REPORT.md (QA 피드백)
...
결과를 ${CLAUDE_PROJECT_DIR}/.harness/feature/<BL-ID>/SPEC.md 로 저장하라.
```
...
```
Read ${CLAUDE_PROJECT_DIR}/.harness/feature/<BL-ID>/SPEC.md
...
결과를 ${CLAUDE_PROJECT_DIR}/.harness/feature/<BL-ID>/SPEC.md 파일 끝에
## Codex Plan Review
섹션으로 추가하라. 기존 내용은 수정하지 마라.
...
- ${CLAUDE_PROJECT_DIR}/.harness/feature/agents/evaluation_criteria.md
- ${CLAUDE_PROJECT_DIR}/.harness/feature/<BL-ID>/SPEC.md
...
전체 기능을 구현하고 결과를 ${CLAUDE_PROJECT_DIR}/.harness/feature/<BL-ID>/output/ 에 저장하라.
완료 후 ${CLAUDE_PROJECT_DIR}/.harness/feature/<BL-ID>/SELF_CHECK.md 를 작성하라.
```
...
- ${CLAUDE_PROJECT_DIR}/.harness/feature/agents/evaluation_criteria.md
- ${CLAUDE_PROJECT_DIR}/.harness/feature/<BL-ID>/SPEC.md
- ${CLAUDE_PROJECT_DIR}/.harness/feature/<BL-ID>/output/ 의 모든 파일

검수 후 결과를 ${CLAUDE_PROJECT_DIR}/.harness/feature/<BL-ID>/QA_REPORT.md 로 저장하라.
```
...
- ${CLAUDE_PROJECT_DIR}/.harness/feature/agents/evaluation_criteria.md
- ${CLAUDE_PROJECT_DIR}/.harness/feature/<BL-ID>/SPEC.md
- ${CLAUDE_PROJECT_DIR}/.harness/feature/<BL-ID>/output/ 의 현재 파일들 (현재 코드)
- ${CLAUDE_PROJECT_DIR}/.harness/feature/<BL-ID>/QA_REPORT.md (QA 피드백)
