...
## 0단계: Task-ID 해결 (v0.6+ — HARNESS-MSC-001 P0-1)

하네스 산출물 충돌 방지를 위해 **모든 경로는 `<TASK-ID>` 네임스페이스로 격리**됩니다. 파이프라인 진입 직후 아래 절차로 Task-ID 를 해결합니다.

> **용어**: `<TASK-ID>` = `<PREFIX>-<digits>` 형태. PREFIX 는 `.harness/config.yaml` 의 `harness.bl_prefix` (기본값 `BL`) 에 의해 결정됨.
> - Portal Hub: `BL-307`
> - 다른 프로젝트: `TICKET-456`, `TASK-7`, `STORY-42` 등

```bash
# 1. 사용자 요청 원문에서 TASK-ID 추출 시도 (HARNESS_BL_PREFIX env 자동 인식)
TASK_ID=$(bash ${CLAUDE_PLUGIN_ROOT}/common/extract_bl_id.sh "[사용자 요청 원문]")

# 2. TASK_ID 에 따라 파이프라인 내 모든 <TASK-ID> 자리표시자를 실제 값으로 치환
#    예) .harness/feature/<TASK-ID>/SPEC.md → .harness/feature/BL-307/SPEC.md (BL prefix)
#       또는                                → .harness/feature/TICKET-456/SPEC.md (TICKET prefix)
```

**해결 우선순위** (`extract_bl_id.sh` 가 자동 수행, `HARNESS_BL_PREFIX` env 기반):
...
**활성 세션 등록 (P0-2)**:
Task-ID 해결 후, plugin 의 `hooks/session-start.sh` 가 이미 `.harness/active-sessions.json` 에 엔트리를 등록했을 것입니다. 추가로 phase 필드를 갱신하려면:

```bash
bash ${CLAUDE_PLUGIN_ROOT}/common/session-registry.sh register "$TASK_ID" "planner"
```
...
⚠️ 타세션 감지 — 동일 프로젝트의 다른 Claude 세션이 .harness/<type>/ 를 점유 중일 수
   있습니다. 진행하시겠습니까? (Task-ID 네임스페이스가 다르면 안전합니다.)
```
